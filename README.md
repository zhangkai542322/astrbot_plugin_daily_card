...
**自动链式调用**：当用户要求"推送到设备/第X页"时，工具会自动提示 LLM 继续调用 `push_image_to_device`，无需手动操作。
...
- 禁止使用 astrbot_execute_python 或其他方式生成中文图片（字体可能不支持）
```
...
LLM 调用 get_weather_card → 生成图片，返回路径
  ↓
工具自动提示: "请立即调用 push_image_to_device(image_path=路径, page_id=1)"
  ↓
LLM 链式调用 push_image_to_device → 推送到设备

用户: "看看北京天气"（不提推送）
  ↓
LLM 调用 get_weather_card → 生成图片，直接展示给用户
```
